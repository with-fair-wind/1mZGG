...
{
    return static_cast<uint16_t>((static_cast<uint16_t>(packet[offset]) << 8U) | packet[offset + 1U]);
}

[[nodiscard]] auto readU32Be(const std::vector<uint8_t>& packet, size_t offset) -> uint32_t
{
    return (static_cast<uint32_t>(packet[offset]) << 24U) | (static_cast<uint32_t>(packet[offset + 1U]) << 16U) |
           (static_cast<uint32_t>(packet[offset + 2U]) << 8U) | static_cast<uint32_t>(packet[offset + 3U]);
}

[[nodiscard]] auto readU32Le(const std::vector<uint8_t>& packet, size_t offset) -> uint32_t
{
    return static_cast<uint32_t>(packet[offset]) | (static_cast<uint32_t>(packet[offset + 1U]) << 8U) |
           (static_cast<uint32_t>(packet[offset + 2U]) << 16U) | (static_cast<uint32_t>(packet[offset + 3U]) << 24U);
}
...
TEST(ImageSender, BuildPacketsMatchesLegacyUdpFraming)
{
    ...
    ASSERT_EQ(packet.size(),
              Dss::Network::ImageSender::PacketHeaderSize + encodedLength +
    ...
    EXPECT_TRUE(std::equal(image.begin(),
                           image.end(),
