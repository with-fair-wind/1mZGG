...
[[nodiscard]] auto readI32Le(const std::vector<uint8_t>& packet, size_t offset)
    -> int32_t {
...
[[nodiscard]] auto readDoubleLe(const std::vector<uint8_t>& packet,
...
TEST(DataExchangeProtocol, BuildsGxtcPacketWithLegacyPackedLayout) {
  ...
  ASSERT_EQ(packet.size(), Dss::Network::GxtcHeaderSize +
                               targets.size() * Dss::Network::GxtcTargetSize);
