...
| 缺口 | 说明 |
|------|------|
| 显式硬件启动入口未接线 | 串口/UDP 服务已注册，但默认不打开硬件；通信页已统一编辑网络端点，图像发送/心跳/诊断/大气接收已通过 `INetworkChannel` 统一控制，数据交换已有专用显式 open/close，显示/曝光/主控/伺服串口已通过 `ISerialChannel` 显式 open/close；Sapera 采集和更细的协议联调命令仍需 UI/联调入口触发 |
