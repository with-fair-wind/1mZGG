...
| 缺口 | 说明 |
|------|------|
| 显式硬件启动入口未接线 | 串口/UDP 服务已注册，但默认不打开硬件；通信页已统一编辑网络端点，图像发送/心跳/诊断/大气接收已通过 `INetworkChannel` 统一控制，数据交换已有专用显式 open/close，显示/曝光/主控/伺服串口已通过 `ISerialChannel` 完成参数编辑和显式 open/close；曝光命令、伺服修正和主控状态回包已有联调入口；字段级接收错误已进入日志页，Sapera 采集、诊断统计聚合和更多协议字段约束仍需继续补齐 |
| 数据交换发送配置待完善 | 跟踪结果已能经桥接器转换为 GXTC/GDCL DTO；`ViewModel` 可编辑网络端点并显式绑定/关闭配置中的 UDP，网络错误事件会显示到状态栏并进入日志页 Error 级别彩色过滤；日志持久化和搜索待补 |
