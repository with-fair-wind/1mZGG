...
| 缺口 | 说明 |
|------|------|
| 显式硬件启动入口未接线 | 串口/UDP 服务已注册，但默认不打开硬件；数据交换已有 ViewModel 显式 open/close，其余串口和网络对象仍需 UI/联调命令触发 open/bind/start |
| 数据交换发送配置待完善 | 跟踪结果已能经桥接器转换为 GXTC/GDCL DTO；`ViewModel::openDataExchange()` 可显式绑定两路 UDP，网络错误事件会显示到状态栏；独立 GXTC/GDCL 配置项和界面参数编辑待补 |
