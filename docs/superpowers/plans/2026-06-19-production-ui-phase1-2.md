...
- [x] 增加配置加载/保存失败测试，覆盖日志默认值与 JSON 往返。
- [x] 增加 rotating sink 配置测试并确认旧 API 无法满足测试。
- [x] 实现 `LoggingConfig` 与可重复配置的滚动文件 sink。
- [x] 运行 `test_config` 和 `test_logger`。
...
- [x] 增加图像会话测试，验证 IMI、RAW/BMP 文件和统计。
- [x] 增加轨迹会话测试，验证 GAE 路径和内容。
- [x] 增加不可写路径测试，验证 `StorageWriteErrorEvent`。
- [x] 实现显式会话 API、统一写入结果和错误事件。
- [x] 运行 storage 相关测试。
...
- [x] 增加事件到诊断状态映射测试并确认失败。
- [x] 订阅网络、串口和存储错误事件，维护线程安全状态。
- [x] 运行诊断测试和网络协议测试。
...
- [x] 扩展 UI 构造测试，要求三个面板存在稳定 objectName。
- [x] 将编辑器状态、工厂函数和刷新连接分别迁入三个 QWidget。
- [x] 将 `setupCommStatusPage()` 收敛为页面组装函数。
- [x] 运行 UI 布局和 ViewModel 测试。
...
- [x] 增加设置加载、输入校验和保存测试并确认失败。
- [x] 实现 SettingsViewModel 并接入 MainViewModel。
- [x] 用路径、日志、光学表单替换占位设置页。
- [x] 运行设置与主窗口测试。
...
- [x] 配置日志、存储事件总线和会话默认值。
- [x] 更新中文迁移状态文档。
- [x] 运行 `cmake --build --preset msvc-debug`。
- [x] 运行 `ctest --test-dir build\\msvc-debug --output-on-failure`。
- [x] 执行 `git diff --check` 和最终代码审查。
