...
__global__ void kernel_short2byte(const uint16_t* src, uint8_t* dst, int min_val, int max_val, float diff, int n)
{
...
__global__ void kernel_sub_frame16(const uint16_t* src1, const uint16_t* src2, uint16_t* dst,
                                   int w, int h, int shift_x, int shift_y)
{
...
__global__ void kernel_reduce_small_dn16(const uint16_t* src, uint16_t* dst, float avg, float std_dev, int n)
{
    int gid = blockIdx.x * blockDim.x + threadIdx.x;
    if (gid >= n)
    {
...
__global__ void kernel_reduce_saturation16(const uint16_t* src, uint16_t* dst,
                                           float avg, float std_dev, float ratio, int n)
{
    int gid = blockIdx.x * blockDim.x + threadIdx.x;
    if (gid >= n)
    {
...
void short_to_byte(const uint16_t* src, uint8_t* dst, int min_val, int max_val, float diff, int n, cudaStream_t stream)
{
    int block = 256;
...
void sub_frame16(const uint16_t* src1, const uint16_t* src2, uint16_t* dst,
                 int w, int h, int shift_x, int shift_y, cudaStream_t stream)
{
    dim3 block(16, 16);
    kernel_sub_frame16<<<grid2d(w, h), block, 0, stream>>>(src1, src2, dst, w, h, shift_x, shift_y);
}

void sub_frame8(const uint8_t* src1, const uint8_t* src2, uint8_t* dst,
                int w, int h, int shift_x, int shift_y, cudaStream_t stream)
{
    (void)src1; (void)src2; (void)dst; (void)w; (void)h; (void)shift_x; (void)shift_y; (void)stream;
...
void reduce_large_dn16(const uint16_t* src, uint16_t* dst, float thresh, int n, cudaStream_t stream)
{
    int block = 256;
    int grid = (n + block - 1) / block;
...
void reduce_small_dn16(const uint16_t* src, uint16_t* dst, float avg, float std_dev, int n, cudaStream_t stream)
{
    int block = 256;
    int grid = (n + block - 1) / block;
...
void reduce_saturation16(const uint16_t* src, uint16_t* dst, float avg, float std_dev, float ratio, int n, cudaStream_t stream)
{
    int block = 256;
    int grid = (n + block - 1) / block;
...
void reduce_saturation8(const uint8_t* src, uint8_t* dst, float avg, float std_dev, float ratio, int n, cudaStream_t stream)
{
    (void)src; (void)dst; (void)avg; (void)std_dev; (void)ratio; (void)n; (void)stream;
