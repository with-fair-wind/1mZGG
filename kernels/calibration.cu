...
__global__ void kernel_remove_bad_line(const uint16_t* in, uint16_t* out,
                                       int w, int h, int detect_w, int detect_h,
...
__global__ void kernel_flat_bias_correct(const uint16_t* in, uint16_t* out,
                                         const uint16_t* flat, const uint16_t* bias,
                                         float flat_avg, float flat_std, int n)
{
    ...
    float corrected = (static_cast<float>(in[gid]) - static_cast<float>(bias[gid])) *
                      flat_avg / fmaxf(1.0f, static_cast<float>(flat[gid]) - static_cast<float>(bias[gid]));
...
void remove_bad_line(const uint16_t* in, uint16_t* out, int w, int h,
                     int detect_w, int detect_h, float threshold_x, float threshold_y, cudaStream_t stream)
{
    dim3 block(16, 16);
...
void flat_bias_correct(const uint16_t* in, uint16_t* out, const uint16_t* flat, const uint16_t* bias,
                       float flat_avg, float flat_std, int n, cudaStream_t stream)
{
    int block = 256;
    int grid = (n + block - 1) / block;
    kernel_flat_bias_correct<<<grid, block, 0, stream>>>(in, out, flat, bias, flat_avg, flat_std, n);
}
