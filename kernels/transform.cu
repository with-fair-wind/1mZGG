...
__global__ void kernel_rotate16(const uint16_t* in, uint16_t* out, float* fout,
                                int w, int h, float cx, float cy, float sin_a, float cos_a)
{
...
__global__ void kernel_crop16(const uint16_t* src, uint16_t* dst,
                              int w_ori, int h_ori, int left, int right, int top, int bottom)
{
...
void rotate16(const uint16_t* in, uint16_t* out, float* fout,
              int w, int h, float cx, float cy, float sin_a, float cos_a, cudaStream_t stream)
{
    dim3 block(16, 16);
    kernel_rotate16<<<grid2d(w, h), block, 0, stream>>>(in, out, fout, w, h, cx, cy, sin_a, cos_a);
}

void crop16(const uint16_t* src, uint16_t* dst,
            int w_ori, int h_ori, int left, int right, int top, int bottom, cudaStream_t stream)
{
    int w_new = w_ori - left - right;
    int h_new = h_ori - top - bottom;
    dim3 block(16, 16);
    kernel_crop16<<<grid2d(w_new, h_new), block, 0, stream>>>(src, dst, w_ori, h_ori, left, right, top, bottom);
}
