...
void var_group_sum16(const uint16_t* src, double avg, double* out, int n, int groups, cudaStream_t stream)
{
    int block = 256;
    kernel_var_group_sum16<<<groups, block, block * sizeof(double), stream>>>(src, avg, out, n);
}

void var_group_sum8(const uint8_t* src, double avg, double* out, int n, int groups, cudaStream_t stream)
{
    (void)src; (void)avg; (void)out; (void)n; (void)groups; (void)stream;
...
void min_max_group16(const uint16_t* src, double* gmin, double* gmax, int n, int groups, cudaStream_t stream)
{
    int block = 256;
    kernel_min_max_group16<<<groups, block, 2 * block * sizeof(double), stream>>>(src, gmin, gmax, n);
}

void min_max_group8(const uint8_t* src, double* gmin, double* gmax, int n, int groups, cudaStream_t stream)
{
    (void)src; (void)gmin; (void)gmax; (void)n; (void)groups; (void)stream;
    // TODO: implement 8-bit variant
}
