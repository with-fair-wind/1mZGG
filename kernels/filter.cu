...
void gaussian_filter16(const uint16_t* in, uint16_t* out, int w, int h,
                       const float* kernel_data, int ksize, cudaStream_t stream)
{
    dim3 block(16, 16);
    kernel_gaussian16<<<grid2d(w, h), block, 0, stream>>>(in, out, w, h, kernel_data, ksize);
}

void gaussian_filter8(const uint8_t* in, uint8_t* out, int w, int h,
                      const float* kernel_data, int ksize, cudaStream_t stream)
{
    dim3 block(16, 16);
...
void median_filter16(const uint16_t* in, uint16_t* out, int w, int h, int ksize, cudaStream_t stream)
{
    dim3 block(16, 16);
...
{
    (void)in; (void)out; (void)w; (void)h; (void)ksize; (void)stream;
    // TODO: port salt filter (outlier replacement)
}

void gray_dilate16(const uint16_t* in, uint16_t* out, int w, int h, int hsize, cudaStream_t stream)
{
    dim3 block(16, 16);
