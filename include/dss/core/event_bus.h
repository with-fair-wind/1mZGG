...
{
    static typename Combiner::result_type get() { return Combiner::emptyResult(); }
};
...
struct EmptyResultAdapter<Combiner, false>
{
    [[noreturn]] static typename Combiner::result_type get()
    {
        throw std::logic_error(
            "Dss::Evt::Event::emit() called with no handlers on a Combiner that requires at least one");
    }
};
...
public:
    ...
    ~ScopedConnection() noexcept { disconnectNoexcept(); }
    ...
    [[nodiscard]] bool isActive() const { return m_active.load(std::memory_order_acquire); }
...
public:
    ...
    [[nodiscard]] bool valid() const { return static_cast<bool>(m_fn); }
...
struct CollectAll
{
    ...
    static result_type emptyResult() { return {}; }
...
struct LastValue
{
    ...
    struct Accumulator
    {
        ...
        void add(R&& val) { lastValue.emplace(std::move(val)); }
        bool shouldStop() const { return false; }
        R finalize()
        {
...
public:
    ...
    [[nodiscard]] std::size_t size() const { return m_core ? m_core->handlerCount() : 0; }
    ...
    [[nodiscard]] std::size_t pendingCount() const { return m_core ? m_core->pendingCount() : 0; }
    ...
    std::vector<ResultType> flush()
    {
        ...
        detail::forEachSafe(batch.begin(), batch.end(), [&](auto& stored) {
            allResults.push_back(
                std::apply([this](auto&... args) { return emit(static_cast<Args>(args)...); }, stored));
        });
        return allResults;
    }
...
private:
    ...
    struct TypedChannel : IChannel
    {
        ...
        std::size_t pendingCount() const override { return m_event.pendingCount(); }
    };
